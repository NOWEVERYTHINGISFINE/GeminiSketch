...
public:
	Gsstime(uint32_t granularity, uint32_t width, uint32_t depth, uint32_t fingerprintLength, uint32_t row_addrs = 4, uint32_t column_addrs = 4);
	~Gsstime();
...
Gsstime::Gsstime(uint32_t granularity, uint32_t width, uint32_t depth, uint32_t fingerprintLength, uint32_t row_addrs, uint32_t column_addrs):
granularity(granularity), width(width), depth(depth), fingerprintLength(fingerprintLength), row_addrs(row_addrs), column_addrs(column_addrs) {
	cout << "Gsstime::Gsstime(granularity: " << granularity 
...
Gsstime::~Gsstime() {
	cout << "Gsstime::~Gsstime()" << endl;
	// delete[] this->value;
	free(this->value);
	vector<vector<node>>().swap(successorAdjacencyList);
}

int Gsstime::getMinIndex(uint32_t* a, int length) {
	uint32_t min = a[0];
	int index = 0;
...
uint32_t Gsstime::getGranularity() const{
	if(this == NULL) {
...
void Gsstime::bucketCounting() {
	cout << "---------------------------------------" << endl;
	cout << "Gsstime bucketCounting(): print bucket..." << endl;
	...
	cout << "Gsstime room_count = " << room_count << ", total room = " << (width * depth * SLOTNUM) << ", space usage is " << 
			(double)room_count / (double)(width * depth * SLOTNUM) * 100 << "%" << endl;
	cout << "Gsstime bucket_count = " << bucket_count << ", total bucket = " << (width * depth) << ", space usage is " << 
...
void Gsstime::insert(string src, string dst, weight_type weight) {
...
weight_type Gsstime::edgeQuery(string src, string dst) {
	uint32_t hash_src = (*hfunc[HASH])((unsigned char*)(src.c_str()), src.length());
	uint32_t hash_dst = (*hfunc[HASH])((unsigned char*)(dst.c_str()), dst.length());
	uint32_t mask = (1 << fingerprintLength) - 1;
	uint16_t fp_src = hash_src & mask;
...
weight_type Gsstime::nodeQuery(string vertex, int type) { // vertex is the ID of the queried node, function for node query.
...
bool Gsstime::reachabilityQuery(string s, string d) {
